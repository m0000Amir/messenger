...
- [ ] все файлы стилей импортировать в какой-нибудь `index.scss`, а его в `index.hbs`. В следующем спринте можно будет раскидать уже по компонентам;
- [ ] `handlebars.config.js` По умолчанию можно ничего не писать, там можно просто настроить папки в которых будут лежать паршиалы, хелперы и тд
- [ ] "dev": "parcel ./index.hbs --port 3000 --no-cache";
- [ ] `/` для handlebars это директория файла;
- [ ] `div` заменить на `li`
- [ ] поставить `flex-grow = 1`
---
